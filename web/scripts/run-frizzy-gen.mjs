...
async function hostImage(path) {
  for (let i = 0; i < 3; i++) {
    try {
      const fd = new FormData(); fd.append("file", new Blob([readFileSync(path)]), "a.png");
      const j = await (await fetch("https://tmpfiles.org/api/v1/upload", { method: "POST", body: fd })).json();
...
console.log("video:", videoUrl?.slice(0, 45), "| avatar:", avatarUrl);

const prompt = `In every frame, completely re-render the subject's face, head, hair, skin and build to be the man in image 1 (short dark hair, facial hair, black t-shirt) — the original person is fully discarded and replaced; do NOT retain any of the original person's face or head.
He performs the exact actions from video 1, in order: he starts with his hair tied up and pulls the tie out letting it fall; sprays a white aerosol can onto his hair and rubs it in; squeezes a white pump bottle onto his head twice and works it in thoroughly; at the 0:06 cut he looks up with perfectly styled curls; runs a hand through the back of his hair; then holds up the two hair products to the camera.
...
while (Date.now() < deadline) {
  ...
  if (["completed", "succeeded", "success", "done"].includes(st)) { const out = pj.results?.[0] ?? JSON.stringify(pj).match(/https?:\/\/[^"]+\.mp4/)?.[0]; console.log("output:", out, "credits:", pj.usage?.credits_used); if (out) { writeFileSync("/tmp/gen_frizzy_edit.mp4", Buffer.from(await (await fetch(out)).arrayBuffer())); console.log("saved /tmp/gen_frizzy_edit.mp4"); } process.exit(0); }
